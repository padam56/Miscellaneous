...
class DQNModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.conv3 = keras.layers.Conv2D(32, (4, 4), (2, 2), activation='relu',use_bias=False)
...
##        self.pooling_layer3 = tf.keras.layers.MaxPool2D(1)
        ...
        if dueling:
            self.v_dense = keras.layers.Dense(hidden_size, activation='relu',use_bias=False,
                                         kernel_initializer=keras.initializers.he_normal())
            self.v_out = keras.layers.Dense(1, activation='softmax',kernel_initializer=keras.initializers.he_normal())
...
target_network = DQNModel(1024, num_actions, True)
