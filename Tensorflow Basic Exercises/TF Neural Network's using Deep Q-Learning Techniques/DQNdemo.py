...
STORE_PATH = 'D:\codewithabhash\Projects\Day02_Tensorflow Neural Networks using Deep Q-Learning Techniques'
MAX_EPSILON = 1
MIN_EPSILON = 0.1
...
env = gym.make("Phoenix-v4") #or KungFuMaster-v0
...
class DQNModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.conv1 = keras.layers.Conv2D(16, (8, 8), (4, 4), activation='relu',use_bias=False) #kernel_size, strides, dilation rate
        ...
        self.adv_dense = keras.layers.Dense(hidden_size, activation='relu',use_bias=False,
                                         kernel_initializer=keras.initializers.he_uniform()) #uniform or normal
...
primary_network = DQNModel(1024, num_actions, True) #instantiating our model




#compile models
primary_network.compile(optimizer=keras.optimizers.Adamax(), loss=tf.keras.losses.Huber())
